...
public class BankAccount {
   ...
   * 입금 처리.
   * @param money 입금액 
   * @throws DepositException - 마이너스 입금 불가. 코드 1000.
   */
  public void deposit(long money) throws DepositException {
    if(money < 0)
      throw new DepositException(money + "원은 입금 할 수 없습니다.", 1000);
    balance += money;
  }
   ...
   */
   public long withdrawal(long money) throws WithdrawalException {
    if(money < 0)
      throw new WithdrawalException(money + "원은 출금 할 수 없습니다.", 2000);
    if(money > balance)
      throw new WithdrawalException(money + "잔액이 부족합니다.", 2001);
    balance -= money;
      return money;
